...
async def dashboard(user: str = Depends(check_auth)):
    ...
    for idx, inst in enumerate(instruments):
        ...
        for tf_name in AVAILABLE_TIMEFRAMES:
            ...
            if is_enabled and tf_cfg:
                ...
                settings_block = f'''
                <div id="st_{idx}_{tf_name}" style="margin:6px 0;padding:6px;background:#2a2a2a;border-radius:4px;display:block;font-size:0.85em;">
            ...
            # 🔥 ФИКС: Убрал label-обертку, добавил onclick с event
            tf_buttons += f'''
    ...
    # 🔥 ФИКС: Правильный JS с event.stopPropagation()
    js_code = """
    <script>
        function toggleTF(event, id) {
            event.stopPropagation(); // 🔥 БЛОКИРУЕМ всплытие к label
...
<head>
    ...
    <style>
        ...
        input[type=number]{{background:#333;border:1px solid #555;color:#fff;padding:4px}}
        .dbg{{background:#222;padding:8px;border-radius:4px;font-size:0.75em;color:#0f0;margin:10px 0;}}
    </style>
</head>
<body>
    <main class="container">
        <h3 style="text-align:center">📊 Инструменты</h3>
        <form action="/save" method="post" id="mainForm">
            {instruments_html}
            <button type="submit" class="big contrast">💾 Сохранить</button>
        </form>
        <article class="card" style="background:#252525;border:1px dashed #666;margin-top:15px;">
            <header><strong>➕ Добавить</strong></header>
            <form action="/add" method="post" style="display:grid;gap:8px;">
                <input type="text" name="t" placeholder="Тикер" required style="padding:8px;" oninput="this.value=this.value">
...
async def save_instruments(request: Request, user: str = Depends(check_auth)):
    ...
    try:
        ...
        logger.info(f"📦 ВСЕГО ПОЛЕЙ: {len(form)}")
        ...
        for idx in range(len(instruments)):
            ...
            # === Главный enabled инструмента ===
            ie_key = f"ie_{idx}"
            ie_val = ie_key in form
            if inst.get("enabled", True) != ie_val:
                inst["enabled"] = ie_val
                logger.info(f"✅ {ticker}: enabled {'вкл' if ie_val else 'выкл'}")
                changes = True
            ...
            for tf in AVAILABLE_TIMEFRAMES:
                ...
                if is_on:
                    ...
                    if existing:
                        # 🔥 ПРОВЕРКА ИЗМЕНЕНИЙ ЦИФР
                        old_hist = existing.get("history_depth_days")
                        old_int = existing.get("update_interval_minutes")
                        old_enabled = existing.get("enabled")

                        if not old_enabled:
                            logger.info(f"✅ {ticker}/{tf}: ВКЛЮЧЕН")
                            changes = True
                        elif old_hist != new_hist:
                            logger.info(f"✅ {ticker}/{tf}: глубина {old_hist} → {new_hist} дн")
                            changes = True
                        elif old_int != new_int:
                            logger.info(f"✅ {ticker}/{tf}: интервал {old_int} → {new_int} мин")
                            changes = True

                        # Обновляем значения (всегда)
                        existing["enabled"] = True
                        existing["history_depth_days"] = new_hist
                        existing["update_interval_minutes"] = new_int
                    else:
                        # Новый таймфрейм
                        logger.info(f"✅ {ticker}/{tf}: ДОБАВЛЕН (глубина={new_hist}, интервал={new_int})")
                    ...
                    # Выключаем существующий таймфрейм
                    for tcfg in inst.get("timeframes", []):
                        if tcfg.get("timeframe") == tf and tcfg.get("enabled"):
                            logger.info(f"✅ {ticker}/{tf}: ВЫКЛЮЧЕН")
                            tcfg["enabled"] = False
                            changes = True
                            break

            # === Авто-удаление если нет активных таймфреймов ===
            #if active_count == 0:
            #    logger.info(f"🗑 {ticker}: 0 активных TF → на удаление")
            #    inst["_del"] = True
            #    changes = True
        ...
        # === Сохранение ===
        if changes:
            logger.info("💾 Сохраняю конфиг...")
            if save_config(cfg):
                logger.info("✅ КОНФИГ СОХРАНЁН")
                return HTMLResponse(content="<script>alert('✅ Сохранено!');location.href='/';</script>")
            return HTMLResponse(content="<script>alert('❌ Ошибка записи!');location.href='/';</script>")
        else:
            logger.info("⚠️ НИКАКИХ ИЗМЕНЕНИЙ НЕ ОБНАРУЖЕНО")
            return HTMLResponse(content="<script>alert('⚠️ Нет изменений');location.href='/';</script>")
...
def start_admin_ui(host: str = "0.0.0.0", port: int = 8000):
    """Запуск UI в фоне"""
    import uvicorn
    logger.info(f"🚀 Admin UI на {host}:{port}")
