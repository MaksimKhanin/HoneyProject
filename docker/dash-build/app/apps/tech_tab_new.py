...
layout = html.Div([
    ...
    dbc.Spinner(
        html.Div(id='tab2-hidden-candles-data', style={'display': 'none'}),
        size="lg", color="primary", type="border", fullscreen=True),

    dbc.Spinner(html.Div(id='tab2-BS-data', style={'display': 'none'}),
                size="lg", color="primary", type="border", fullscreen=False),
    dbc.Spinner(html.Div(id='tab2-CF-data', style={'display': 'none'}),
                size="lg", color="primary", type="border", fullscreen=False),
    dbc.Spinner(html.Div(id='tab2-INC-data', style={'display': 'none'}),
                size="lg", color="primary", type="border", fullscreen=False),
    dbc.Spinner(html.Div(id='tab2-KM-data', style={'display': 'none'}),
                size="lg", color="primary", type="border", fullscreen=False),

    dbc.Row(dbc.Col(html.Div(id="tab2-slider",
...
@app.callback(
    ...
    [Input('tab2-ticker-selector', 'value'),
     ...
     Input('input-BB', 'value'),
     Input('tab2-hidden-candles-data', 'children')])
def update_graph(ticker, date_range, mv1, mv2, bb, main_data):

    if not main_data or not date_range or not ticker:
        raise PreventUpdate
    df = pd.read_json(main_data)
    df = df.astype({
        "timestamp": "int64",
